...
final class FifoLab
{
    ...
    public static function install(): array
    {
        ...
        $inputs=[];
        foreach (self::INPUTS as $name=>$type) $inputs[$name]=self::create($name,$root,2,static fn()=>IPS_CreateVariable($type));
        foreach ($inputs as $name=>$id) SetValue($id,self::INPUTS[$name]===1 ? 0 : false);
        ...
        $config=self::configuration($inputs);
    ...
    {
        if ($root<=0 || (IPS_GetObject($root)['ObjectIdent']??'')!==self::IDENT || IPS_GetParent($root)!==0) throw new RuntimeException('Invalid lab root.');
        $id=self::child('Manifest',$root); if(!$id)throw new RuntimeException('Run lab installer first.');
        $raw=GetValue($id);if(!is_string($raw)||strlen($raw)>8192)throw new RuntimeException('Lab manifest exceeds its size limit.');
    ...
    private static function validate(array $m, bool $allowControlDraft = false): void
    {
        ...
        if(array_keys($m['inputs'])!==array_keys(self::INPUTS)||count($m['done'])!==3)throw new RuntimeException('Lab input inventory changed.');
        ...
        foreach(self::INPUTS as $name=>$type) if((IPS_GetVariable($m['inputs'][$name])['VariableType']??-1)!==$type)throw new RuntimeException('Lab input type changed.');
        ...
        $expected=self::configuration($m['inputs']);
    ...
    public static function execute(int $root,array $params): void
    {
        ...
        try {
            ...
            foreach($m['inputs']as$name=>$id)SetValue($id,self::INPUTS[$name]===1?0:false);
