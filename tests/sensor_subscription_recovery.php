...
$m->pending['EnableInputFifo']=true;$m->pending['EnableFifoFailureCapture']=true;$m->pending['EnableFifoShadow']=true;
$m->attributes['FifoOwned']=true;$m->attributes['FifoTestActive']=true;
$GLOBALS['calls']=[];$GLOBALS['variables'][101]=true;$m->MessageSink(2,101,VM_UPDATE,[true,true,false]);
subscriptionCheck(!method_exists($m,'RunInputFifo')&&!method_exists($m,'GetInputFifoReport')&&!method_exists($m,'StartFifoShadow'),'Stable runtime has no experimental FIFO/shadow API dependency');
subscriptionCheck($m->GetValue('Status')===true,'Retained experimental settings/attributes cannot route stable input through experimental evaluator');
subscriptionCheck(!isset($m->properties['EnableInputFifo'],$m->properties['EnableFifoFailureCapture'],$m->properties['EnableFifoShadow']),'Stable Create registers no experimental settings');
echo "Stable subscriptions: $checks checks passed. Native update recovery still needs confirmation.\n";
