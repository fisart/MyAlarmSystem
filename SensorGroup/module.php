    ...
    public function RequestAction($Ident, $Value)
    {
        ...
        switch ($Ident) {
            ...
            case 'UpdateGroupDispatch': {
                    ...
                    $master = $this->GetBufferedSectionList('GroupDispatchBuffer', 'GroupDispatch');

                    if ($this->ReadPropertyBoolean('DebugMode')) {
                        $this->LogMessage("DEBUG: UpdateGroupDispatch incoming=" . json_encode($incoming), KL_MESSAGE);
                        $this->LogMessage("DEBUG: UpdateGroupDispatch master_before=" . json_encode($master), KL_MESSAGE);
                    }

                    // Normalize incoming rows only by structural meaning.
                    // Do NOT validate against current dropdown options here.
                    $incomingNormalized = [];
                    $incomingSeen = [];
                    ...
                    // Preserve-heavy first pass:
                    // - if incoming has no meaningful rows, keep current buffer unchanged
                    // - if incoming looks like a full/expanded snapshot, accept it
                    // - otherwise merge by union, preferring preservation over deletion
                    if (count($incomingNormalized) === 0) {
                        $result = $master;
                    } elseif (count($incomingNormalized) >= count($master)) {
                        $result = $incomingNormalized;
                    } else {
                        $result = $master;
                        $existingKeys = [];
                        ...
                        foreach ($result as $row) {
                            ...
                            $k = trim((string)($row['GroupName'] ?? '')) . '::' . (int)($row['InstanceID'] ?? 0);
                            $existingKeys[$k] = true;
                        }

                        foreach ($incomingNormalized as $row) {
                            $k = $row['GroupName'] . '::' . $row['InstanceID'];
                    ...
                    $this->WriteBufferedSectionList('GroupDispatchBuffer', array_values($result));

                    if ($this->ReadPropertyBoolean('DebugMode')) {
                        $this->LogMessage("DEBUG: UpdateGroupDispatch master_after=" . json_encode(array_values($result)), KL_MESSAGE);
                    }
