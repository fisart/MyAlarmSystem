...
## fifo.6: supervised first-failure diagnostic review

Artur stopped the vendor/thread-boundary approach and explicitly accepted lost messages/overlapping old evaluations during switching for this test. Independent read-only review approves the narrow opt-in diagnostic policy, not reliable production FIFO operation. The historical guard is retained, normal activation remains guarded, and no per-sensor merging/debounce or Module 2/3/watchdog changes are included.

Review found and corrected repeated fault publication from paused malformed inputs, generic fault-latch contamination of a new test, repeated persistent writes/report mutex use, and baseline publication after a fault during the publication-lock acquisition. The final patch pauses input/control admission and recovery, drains only a trustworthy retained prefix, allows Disable+Apply past an unusable paused queue, and retains one bounded first-failure record through disabling/recreation. Contended first capture is explicitly unavailable and cannot be replaced by a later precise cause. Form/Mermaid show pause and the required action.

Seven local suites pass **448 checks**: safety129, probe44, shadow64, FIFO89, form24, passive39 and first-failure59. The independent reviewer separately reran FIFO89 and first-failure59 and found no remaining blockers; changed PHP lint and whitespace checks pass. Native failure cause, heavy concurrency, CPU/resident RAM and production acceptance remain unresolved. There is no automatic test expiry or fallback; Artur must remain present and disable live FIFO and diagnostic mode, then Apply and verify fresh heartbeat. See [test instructions](module1-fifo-first-failure-test.md).

## Native script timing lab

Artur resumed diagnostics and proposed a script FIFO emulator with separate synthetic inputs/timing scenarios. The reviewed implementation instead drives actual Module 1 through native variable updates, in a dedicated eight-input/seven-class lab instance with no dispatch routes, bedroom/vault target or production receiver access. Six bounded scenarios cover sequential/interleaved/concurrent inputs, unchanged refresh plus ordinary token/reset, baseline acquisition activity and deliberate lab-only queue mutex contention. No module runtime changes or copied FIFO are introduced.
...
## fifo.9 scheduling independent review — 2026-09-13

A separate read-only reviewer independently passed **31 scheduling /109 runtime /24 timing /59 first-failure checks** and approved **supervised native lab experiment only**, finding no blocker. Full local suite580checks,22workflow PHP syntax files, clean whitespace. Native fifo.9 improvement and shared-system load remain unverified.

Confirmed applied FifoTestActive gating; initial idle wake/ordinary continuation50ms; one-time10msnonempty diagnostic continuation at serialized batch shutdown, no resets while already fast; pending recovery retains fast timer. Worker ownership,32record/soft20msbudget, queue waits/capacities and equal ordinary sensor/heartbeat path unchanged. Tests cover idle/admission race, busy-owner yield, unapplied flag isolation, admitted-prefix deferred Disable, paused-prefix draining, false/throwing native timer update and no replay. No timer-per-event scripts, spinning, larger batch, log/archive/history or growing state.

Official [GetTimerInterval documentation](https://www.symcon.de/en/service/documentation/developer-area/sdk-tools/sdk-php/module/gettimerinterval/) confirms integer milliseconds and Symcon>=5.2; [SetTimerInterval](https://www.symcon.de/en/service/documentation/developer-area/sdk-tools/sdk-php/module/settimerinterval/) confirms millisecond update /Boolean success. This establishes signature compatibility, not scheduling precision/concurrency/fairness.10msrequests can arrive while a20ms+ batch owns the worker;1msworker lock wait limits the losing request and prevents parallel evaluation. The remaining native risk is concentrated CPU/thread load and shared heartbeat/service delays; serialized bytes/timing are not CPU/residentRAM evidence.

Next native gate is the same MyAlarmFifoLoadLab54312 concurrent_flicker70changes plus real heartbeat35750history covering run. Later forced40msadmission contention and per-source/order/value/COUNT/ONCE behavior remain required before a supervised production trial. No main merge, production FIFO approval, restart or legacy guard reset.
