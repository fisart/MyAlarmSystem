...
Native synthetic-input lab code is published at `435c5b03baae85211b9350284c122686d82cc80c`. The current follow-up is `Version2.14.0-fifo.7`: genuine-idle-only worker wake and a single 10 ms queue-lock wait for admission/essential worker commits. Independent review approved fifo.7 for supervised lab re-test after independently passing runtime 109, first-failure 59 and lab 34 checks; native fifo.7 sequential, two interleaved runs, concurrent_flicker and refresh_noise pass aggregate comparison; requested baseline refresh completed, and intentional admission contention captured/pause/cleanup worked. The initial lab sequence is complete; throughput/load acceptance is pending. **Native fifo.6 concurrent_flicker captured admission contention; fifo.7 processed all 70 concurrent changes without a current fault, with maximum recorded lag 1.348 seconds. The original production legacy event-loss cause remains unresolved.**
