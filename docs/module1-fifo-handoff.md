...
Current follow-up: **Version2.14.0-fifo.12**, separate default-off `AllowFifoTrialCutover` for a supervised production trial on SensorGroup **23172**. It accepts the user's uncertain switching overlap without resetting the retained guard, keeps normal automatic recovery, and uses native-tested10ms pending continuation/50ms idle wake. Failure capture remains separate; conflicting modes cannot activate FIFO. Modules2/3/watchdog unchanged; heartbeat ordinary input. No automatic expiry/fallback, no main/permanent-production approval.

**Next native action:** update design/module1-fifo to fifo.12, then follow [supervised production trial](module1-fifo-production-trial.md). Enable live FIFO plus supervised trial on **production23172**, keep shadow/failure capture OFF, COMMIT, provide first running report. Require actual enabled/ready/trial.active, automatic_recovery true, test inactive, no unknown inputs/current fault. Retained overlap warning is expected. Keep trial supervised and manually disable live FIFO+trial afterward; confirm actual ownership off/timer0, not merely requested flags. No restart/reinstall/guard reset. Separate heartbeat_overlap lab deferred by user; do not automatically request extra watchdog cycles.

Latest production preparation: all61pendingClassIDs were missing but other active-snapshot class settings matched. Supported COMMIT healed IDs by unique names from active baseline. User confirmed IPS_HasChanges23172=false, FIFO/test ownershipOFF, timer0, guardretainedtrue, no current fault/unknowninputs. Stable existing evaluator is active. Publication/independent review/CI checkpoint in PR Conversation.

Local full workflow **692 checks / thirteen suites**,27PHPsyntax/all40commands. Added trial suite42checks covers applied/draft permissions, guard visibility, normal recovery/prefix/no replay, ordinary token/reset values through receiver stubs,10ms scheduling, deferred rollback and mode conflicts. No new per-event scripts, logs, polling, history or timers. Trial adds bounded scalar checks at Apply/batch/report boundaries and one latched switching warning. CPU/resident RAM and synchronous production receiver costs remain unmeasured.

Native fifo.10 semantic session94c9625cbbb37b92 passed all10assertions,9/9/empty/actualOFF,maxlag153.437ms; captured sequence/mirror values, COUNT1/1/2, ONCE create/clear and token/reset correct. Native fifo.9 larger concurrent70/70 with maxlag807.833ms versus fifo.8 final2527.698ms; forced contention detection/pause/cleanup passed zero admissions, no nonempty-prefix failure-drain proof. Ten earlier real watchdog cycles allOK but did not overlap burst. Original production legacy event-loss cause unresolved; downstream production FIFO/routing/house-state gates later.

Local fullworkflow **650 checks / twelve suites**,26PHPsyntax/all38commands, clean whitespace. Heartbeat suite35 / lab59 / verification33. New helper bounded reads/history; producer completion adds two timestamps only. No new per-event scripts/logging/archive/timers. Real RunCycle is synchronous and can include normal cycle/history semaphore waits, <=3s permitted pulse sleep and configured notification I/O; existing lab polling deadlines cannot cancel/bound an invoked production cycle. Stop before invocation prevents request; let started ordinary cycle/reset finish. API exceptions explicit/no pass/lab cleanup. Scope shared-resource producer execution overlap and confirmed callbacks, not continuous CPU load/CPU-RAM or real heartbeat through production FIFO. Independent review/publication/CI checkpoint in PR Conversation and [review](module1-fifo-review.md).
...
- Production live FIFO, shadow and failure capture stay disabled during isolated lab tests. The separately authorized supervised production trial uses live FIFO plus its explicit trial option, with shadow/failure capture OFF.
