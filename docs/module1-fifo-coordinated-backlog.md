# Deferred coordinated FIFO architecture

Status: superseded as the active PR #3 scope by Artur on 2026-09-13. This document preserves the previously reviewed Modules 1–3 proposal for future work. Its guarantees, protocol changes, budgets and implementation prerequisites do not apply to the current Module 1-only change. The later approvals for unattended recovery, webpage visibility and intrusion cancellation remain preferences for that future work.
